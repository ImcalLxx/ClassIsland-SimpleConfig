...
class JsonManager:
    ...
    def __init__(self, uuidFilePath: str = "./data/uuid.cic") -> None:
        """
        初始化
    ...
    def classPlan2Dict(self, classTable: ClassTable, timeTable: TimeTable) -> dict:
        """
        ...
        weekcount1 = ((daydiff % 2) + timeTable.weekOffset1) % 2       # 单双周
        weekcount2 = ((daydiff % 3) + timeTable.weekOffset2) % 3       # 3周轮换
    ...
    def generateOverAllDict(self, classTable: ClassTable, timeTable: TimeTable) -> None:
        ...
        # 进行检查
        if self.classPlan2Dict(classTable, timeTable) == {}:
            logger.error("写入课表配置文件终止")
            self.overAllDict = {}
            return

        retDict["TimeLayouts"] = self.timeLayouts2Dict(timeTable)
        retDict["ClassPlans"] = self.classPlan2Dict(classTable, timeTable)
